#!/bin/bash
# Activate all 23 protocol features on the Verarta production chain.
# Run this on the server where cleos and the nodeos containers are accessible.
#
# Production chain produces blocks continuously at 5s intervals (no pace controller).
set -euo pipefail
...
wait_for_block() {
  ...
  local old_block
  old_block=$(curl -sf http://localhost:8000/v1/chain/get_info | python3 -c "import sys,json; print(json.load(sys.stdin)['head_block_num'])")
  for i in $(seq 1 30); do
    sleep 2
    local new_block
    new_block=$(curl -sf http://localhost:8000/v1/chain/get_info | python3 -c "import sys,json; print(json.load(sys.stdin)['head_block_num'])" 2>/dev/null) || continue
...
count_activated() {
  curl -sf -X POST http://localhost:8000/v1/producer/get_supported_protocol_features \
...
echo "=== Verarta Production Chain: Protocol Feature Activation ==="
echo ""
...
echo "Switching to Savanna finality requires setting a finalizer policy with BLS keys (separate step)."
